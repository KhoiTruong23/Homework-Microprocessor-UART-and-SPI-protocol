;
; Homework_SPI_Master.asm
;
; Created: 11/18/2023 2:43:37 PM
; Author : minhk
;
...
MAIN: 
	...
	OUT PORTA,R16 ;
	CBI DDRD,2
	LDI R16,(1<<SS)|(1<<SCK)|(1<<MOSI) ; SPI
	OUT DDRB,R16
	SBI PORTB,SS ; STOP SPI
	LDI R16 ,(1<<SPE0)|(1<<MSTR0)|(1<<SPR00)  ;SPI Master,SCK=Fosc/16, SPI ENABLE, SPI MODE 0
	OUT SPCR0,R16
START: 
	SBI PORTB,SS
	SBIS PIND,2
	...
	call USART_SendChar
	call screen
	RJMP START ; LOOP BACK
screen:
	ldi r16,0x84 //key
	call LCD_Send_Command
	mov r16,r18
	call LCD_SEND_DATA
;SPI_TRANSACTION  how slave and master communicate 
;Input: R17	store data we will send to slave
;Output: R18 store data we receive from slave
SPI_TRANSACTION:
	OUT SPDR0,R17 ;	MASTER TO SLAVE 
CHECK_FLAG:
	IN R16,SPSR0 ; CHECK FLAG SPIF0
	SBRS R16,SPIF0 ;
	RJMP CHECK_FLAG  ;
	IN R18,SPDR0 ; SLAVE TO MASTER
	RET
